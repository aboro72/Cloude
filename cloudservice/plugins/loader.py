...
class PluginLoader:
    ...
    @staticmethod
    def _normalize_settings_config(manifest: Dict[str, Any]) -> Dict[str, Any]:
        """Return a safe plugin settings config dict."""
        settings_config = manifest.get('settings') or {}
        return settings_config if isinstance(settings_config, dict) else {}
    ...
    def discover_plugins(self) -> List[Dict[str, Any]]:
        ...
        for item in self.PLUGINS_DIR.iterdir():
            ...
            try:
                ...
                settings_config = self._normalize_settings_config(manifest)
                has_settings = settings_config.get('has_settings', False)
                settings_schema = settings_config.get('schema') or {}
                ...
                if created:
                    ...
                    if plugin.version != manifest['version'] or (plugin.settings_schema or {}) != settings_schema:
    ...
    def load_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            # apps.populate() also initializes app_config.models via import_models().
            # Without that, internal Django code can hit app_config.models.values()
            # while models is still None, which breaks plugin activation.
            ...
            if getattr(app_config_instance, 'models', None) is None:
                app_config_instance.models = {}
            apps.app_configs[module_name] = app_config_instance
            app_config_instance.import_models()
            apps.clear_cache()
            ...
            plugin.save(update_fields=['status', 'enabled', 'activated_at', 'error_message', 'module_name'])
            ...
            plugin.save(update_fields=['status', 'error_message'])
    ...
    def unload_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            plugin.save(update_fields=['enabled', 'status'])
    ...
    def register_plugin_hooks_only(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            # Mirror the parts of Apps.populate() that plugins commonly rely on.
            app_config_instance = app_config_class(module_name, plugin_module)
            app_config_instance.apps = apps
            if getattr(app_config_instance, 'models', None) is None:
                app_config_instance.models = {}
            apps.app_configs[module_name] = app_config_instance
            app_config_instance.import_models()
